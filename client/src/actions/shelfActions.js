...
export const updateMyShelf = () => async dispatch => {
  try {
    ...
    dispatch({
      ...
      payload: myShelf.reverse() || []
    });
  } catch (error) {
    console.log("TCL: error", error)
    console.error('/api/myShelf failed with error: ' + error);
...
export const searchGoodreads = (query, intro) => async dispatch => {
  const searchData = {
    query,
...
export const postReview = (bookID, review) => async dispatch => {
  const reviewData = {
    bookID,
    review
  };
