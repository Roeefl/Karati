...
class MySwipes extends React.Component {
  ...
  renderContent() {
    ...
    if (!this.props.swipeHistory) {
      return <Spinner message="Fetching your swipe history..." />;
    }

    const swipes = this.props.swipeHistory.map(swipe => {
      console.log(swipe);
      return <SwipeCard swipe={swipe} key={swipe._id} />;
    });

    return <div className="swipes-grid ui divided items">{swipes}</div>;
  }
    ...
    return (
      <div className="my-swipes ui container">
        <Message
          color="violet"
          lines={[
            `You've swiped on ${this.props.swipeHistory.length ||
              0} books so far`
          ]}
        />

        <div className="ui centered grid">
          <div className="ui ten wide column">{this.renderContent()}</div>
        </div>
      </div>
    );
  }
