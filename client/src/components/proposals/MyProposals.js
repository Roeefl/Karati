...
class MyProposals extends React.Component {
  ...
  renderContent() {
    if (!this.props.myProposals) {
      return <Spinner message="Fetching Proposals..." />;
    }

    if (this.props.myProposals.error) {
      return <Message color="red" lines={[this.props.myProposals.error]} />;
    }

    if (this.props.myProposals.length <= 0) {
      return <Message color="red" lines={[errors.NO_PROPOSALS]} />;
    }
      ...
      <div className="ui grid proposals-grid">
        <div className="five wide column filter-col">
          <div className="proposals-container">
            ...
            <div className="proposal-cards">{proposalCards}</div>
          </div>
        </div>

        <div className="eleven wide column chat-col">
          <ProposalChat proposal={this.props.currentProposa} />
        </div>
      </div>
    );
  }
