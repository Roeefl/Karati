...
class MyWishlist extends Component {
  ...
  renderWishlist() {
    if (this.props.wishlist === false) {
      return <Spinner message="Fetching Your Wishlist..." />;
    }

    if (this.props.wishlist.error) {
      return <Message color="red" lines={[this.props.wishlist.error]} />;
    }
  ...
  render() {
    return (
      <div className="my-wishlist ui container">{this.renderWishlist()}</div>
    );
  }
}
