  ...
  renderList() {
        ...
        return (
          <div className="item" key={`${book._id}`}>
            ...
            <div className="content">
              ...
              <div className="description">{book.title}</div>
            </div>
          </div>
        );
      });

    if (swipeList.length < 1) return;
    ...
    return (
      <React.Fragment>
        ...
        <div className="ui items">{swipeList}</div>
      </React.Fragment>
    );
  }
  render() {
    return <div className="swipe-item-list">{this.renderList()}</div>;
  }
}
