...
class MatchesWithUser extends React.Component {
  ...
  componentDidMount() {
    this.props.setCurrentComponent({
      ...
      secondary:
        'Pick which one of your books you are willing to swap with which one of theirs',
      icon: 'exchange'
    });
  ...
  selectBook = (bookId, myBook) => {
    if (myBook) {
      this.setState({
        myPickedBook: this.props.matchesWithUser.myBooks.find(
          book => book._id === bookId
        )
      });
    } else {
      this.setState({
        hisPickedBook: this.props.matchesWithUser.hisBooks.find(
          book => book._id === bookId
        )
      });
    }
  ...
  proposeSwap = () => {
    ...
    this.props.proposeSwap(
      this.props.userData._id,
      this.props.matchesWithUser.ownerInfo._id,
      this.state.hisPickedBook._id,
      this.state.myPickedBook._id
    );
  };
  ...
  renderBookCard = (book, myBook) => {
    ...
    let currentlyPicked =
      (myBook &&
        this.state.myPickedBook &&
        this.state.myPickedBook._id === book._id) ||
      (!myBook &&
        this.state.hisPickedBook &&
        this.state.hisPickedBook._id === book._id);
  ...
  renderContent() {
    ...
    if (
      !this.props.matchesWithUser ||
      Object.keys(this.props.matchesWithUser).length === 0
    ) {
      return <Spinner message="Retrieving matches with user..." />;
    }

    const myBooks = this.props.matchesWithUser.myBooks.map(book =>
      this.renderBookCard(book, true)
    );

    const hisBooks = this.props.matchesWithUser.hisBooks.map(book =>
      this.renderBookCard(book, false)
    );
    ...
    return (
      <div className="ui container">
        ...
        <h2 className="ui centered header red">
          ...
          Here are your swap options with{' '}
          {this.props.matchesWithUser.ownerInfo.username}
        </h2>
        ...
        <h2 className="ui centered header red">
          ...
          {this.props.matchesWithUser.ownerInfo.username}'s books that you
          marked as interested in:
        </h2>
        <div className="ui link cards four column grid">{hisBooks}</div>

        <div className="ui divider"></div>

        <h2 className="ui centered header red">
          <i className={`${icons.BOOK} icon green`} />
          Your books that {this.props.matchesWithUser.ownerInfo.username} is
          interested in:
        </h2>
        <div className="ui link cards four column grid">{myBooks}</div>
      </div>
    );
  }

  render() {
    return <div className="matches-with-user">{this.renderContent()}</div>;
  }
}
...
function mapStateToProps(state, ownProps) {
  ...
  return {
    ...
    matchesWithUser: state.myMatches
      ? state.myMatches.find(
          dataObject =>
