...
class Book extends React.Component {
  componentDidMount() {
    if (
      this.props.selectedBookFromDB &&
      this.props.bookId &&
      this.props.bookId !== this.props.selectedBookFromDB._id
    ) {
  ...
  componentWillUnmount() {
    // SAFETY
    if (
      this.props.selectedBookFromBrowse &&
      this.props.selectedBookFromBrowse.bookID ===
        this.props.selectedBookFromDB._id
    ) {
  ...
  swipeBook = liked => {
    this.props.swipeBook(
      liked,
      this.props.selectedBookFromBrowse.bookID,
      this.props.selectedBookFromBrowse.ownerID,
  ...
  renderReview() {
    ...
    return (
      <BookReviewByCurrentUser
        book={this.props.selectedBookFromDB}
  ...
  renderSwapComplete() {
    return (
      <div className="ui container">
        <Message color="green" lines={['You got it boss!']} />
  ...
  renderActions() {
    if (!this.props.selectedBookFromDB) return;

    if (!this.props.userData || !this.props.userData.ownedBooks) return;

    const onShelf = this.props.userData.ownedBooks.find(
      book => book.bookID === this.props.selectedBookFromDB._id
    );
    ...
    if (onShelf)
      return (
        <ShelfBookActions
          ...
          user={this.props.userData}
          onShelf={onShelf}
          refreshBook={this.props.fetchUser}
        />
      );

    if (!this.props.selectedBookFromBrowse) return;
    ...
    if (
      ...
      if (
        this.props.userData.swipes.find(
          swipe => swipe.bookID === this.props.selectedBookFromBrowse.bookID
        )
      )
  ...
  renderAllReviews() {
    ...
    return <BookReviews book={this.props.selectedBookFromDB} />;
    // }
  }
