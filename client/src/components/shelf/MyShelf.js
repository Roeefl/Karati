...
class MyShelf extends React.Component {
    ...
    if (this.props.myShelf.length <= 0) {
      return (
        <Message
          color="red"
          lines={[
            `You have not added any books to your shelf yet. Bad user! *System spanks ${
              this.props.userData ? this.props.userData.username : 'John Doe'
            }*`
          ]}
        />
      );
  ...
  renderContent() {
    ...
    if (myShelf === false) {
      return <Spinner message="Fetching Your Shelf..." />;
    }

    if (myShelf.error) {
      return <Message color="red" lines={[myShelf.error]} />;
    }

    const shelfBooks = myShelf.map(book => {
      const trimmedDesc = book.description.substring(0, 200);
    ...
    return (
      <div>
        ...
        <div className="my-shelf-grid ui link cards grid">{shelfBooks}</div>
      </div>
    );
  }

  render() {
    return <div className="my-shelf ui container">{this.renderContent()}</div>;
  }
}
