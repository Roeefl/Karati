...
class FrontPage extends React.Component {
  ...
  renderFeed(feed, dark) {
    if (!feed || !feed.data) {
      ...
      return <Spinner message={spinnerMsg} />;
    }
    ...
    if (feed.data.length === 0) {
      return (
        <Message
          color="red"
          lines={[`No ${feed.title} found in the system. Super strange`]}
        />
      );
    }
    ...
    return <div className="ui link cards grid">{feedColumns}</div>;
  }
  ...
  renderBookFeeds() {
    ...
    const feeds = this.props.feeds.map(feed => {
      ...
      return (
        <div
          className={`books-feed ui vertical stripe ${bgColor}`}
          key={feed.title.colored}
        >
          ...
          <div className="ui middle aligned stackable grid container">
            <div className="center aligned row rtl app-logo">
              <div className="column">
                <h1 className="ui header">{getString(language, 'slogan')}</h1>
                <h3 className="ui header">
                  {getString(language, 'subSlogan')}
                </h3>
              </div>
            </div>
            <div className="centered row">
              ...
              <div className="center aligned four wide column">
                <h1>{getString(language, 'preview.myShelf.primary')}</h1>
                <em>{getString(language, 'preview.myShelf.secondary')}</em>
                <Link to="/myShelf">
                  <div className="ui huge violet labeled icon button">
                    <i className="icon zip white large" />
                    {getString(language, 'myShelf.button')}
                  </div>
                </Link>
              </div>
              ...
              <div className="four wide center aligned column">
                <h1>{getString(language, 'preview.nearby.primary')}</h1>
                <em>{getString(language, 'preview.nearby.secondary')}</em>
                <Link to="/books/browse">
                  <div className="ui huge orange labeled icon button">
                    <i className="icon delicious white large" />
                    {getString(language, 'nearby.button')}
                  </div>
                </Link>
              </div>
              <div className="one wide column" />
